...
重要词语第一次在正文的相关知识点中参与推导时，按照以下顺序展开：
...
3. **不循环解释**：不能用另一个尚未解释的术语完成定义；若无法避免，必须立即补充该术语，或从当前正文位置链接到已经给出的定义。
...
9. **写清输入、动作、输出和验证方法**：过程类概念不能只写“负责处理、优化并生成某种表示”，必须说明它拿到什么具体代码或文件、改变了什么、产出什么，以及学习者用哪条命令或哪个现象验证。
10. **用同一个具体对象贯穿解释**：优先选择一行代码、一个对象、一个目标文件或一条诊断，连续展示它在各步骤中的变化；不能每一段都换一个新术语却没有可追踪对象。
11. **给出求职表达**：核心主题至少提供一个 30～60 秒口述答案或典型追问，答案必须能由前面的实验支撑，不能单独背诵。

#### 3.3 正文解释与术语索引必须互相配合

每份讲义在正文前设置表格形式的“关键术语索引”，但它只承担查阅和复习功能，不能代替正文中的教学过程。学习者不需要先背完整个索引，才可以继续阅读正文。索引表第一列列出可点击的术语，第二列给出一句话直观解释；点击术语后跳转到正文后的完整术语卡。

执行规则如下：

1. **核心术语在当前位置解释**：某个术语第一次真正参与知识推导时，在该段正文中先给新手可复述的解释，再给准确专业定义，并立即结合当前代码、执行过程或构建产物说明。
2. **补充术语允许链接查阅**：若某个次要术语的术语卡已经足够，正文首次有效使用时必须链接到对应的 Markdown 标题，例如 `[重定位](#术语-d04-13-重定位)`，使学习者可以原地跳转。
3. **索引统一使用两列表格**：第一列使用 `[术语](#对应标题)` 形成可点击入口，第二列只放一句话直观解释，使索引紧凑且便于快速扫描。
4. **术语卡放在不打断主线的位置**：完整术语卡原则上放在正文之后的附录；不要在正文开始前连续堆放几十个定义。学习者应先跟随具体问题和示例建立理解，再按需跳转查阅。
5. **使用真正的 Markdown 标题作为目标**：每个术语卡使用六级标题，例如 `###### 术语-d04-13-重定位`。标题只使用中文、英文小写字母、数字和连字符，正文及索引中的片段必须与标题文字完全一致。
6. **禁止伪兼容锚点**：不得使用 `<a id="..."></a>` 作为 Obsidian 跳转目标，也不得把表格中的单独一行作为跳转目标。Markdown 表格只承担导航，真正的目标必须是文档中实际存在的 Markdown 标题。
7. **链接不能掩盖讲解缺失**：仅当索引定义足以支撑当前段落时才使用链接；当前推导依赖该概念的规则、边界或易混点时，必须在正文直接解释。
8. **避免无效术语堆积**：若一个词在正文中没有实际使用或解释位置，就不应仅为了扩大术语索引而列为“关键术语”。
9. **重复出现不必重复定义**：首次建立正确概念后，后文可以直接使用；需要回顾时再次链接即可。
10. **发布前验证真实目标**：检查每个 `#...` 片段都存在同名 Markdown 标题，并确认不存在旧式 HTML 锚点或孤立链接；字符串相同不等于 Obsidian 已能索引。

每张完整术语卡至少包含：

- **术语名称**：给出中文名称，必要时附英文名称或实际 C++ 拼写。
- **新手先这样理解**：用一句话建立正确直觉。
...
一段话即使技术上没有明显错误，只要读者仍无法回答“它到底拿什么做了什么”，就不能视为合格讲解。

以下写法必须整改：

- 连续出现“分析、处理、优化、生成低层表示、进行抽象”等词，却没有展示具体输入和输出
- 用两个以上尚未解释的新术语去定义另一个新术语
- 只列职责清单，不说明其中至少一个职责怎样作用于一行真实代码
- 只给命令，不说明命令前后的文件内容或状态发生了什么变化
- 只给类比，没有准确边界；或只给专业定义，没有可观察实例
- 面试答案脱离实验，学习者只能逐字背诵而不能自行推导

过程类知识发布前必须通过“五问检查”：

| 检查问题 | 合格证据 |
|---|---|
| 它为什么存在？ | 给出没有它会遇到的具体问题 |
| 它拿到什么？ | 指明代码、对象、文件、数据或程序状态 |
| 它具体改变什么？ | 跟踪一个可见对象的变化，不只列抽象动词 |
| 它交出什么？ | 指明输出产物或状态及其用途 |
| 我怎样验证？ | 提供命令、运行结果、诊断、内存状态或对比实验 |

核心机制还要补充第六问：**面试官可能怎样问，我怎样用自己的实验在 30～60 秒内回答？**
