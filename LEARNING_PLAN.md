...
每份讲义在正文前设置表格形式的“关键术语索引”，但它只承担查阅和复习功能，不能代替正文中的教学过程。学习者不需要先背完整个索引，才可以继续阅读正文。索引表第一列列出可点击的术语，第二列给出一句话直观解释；点击术语后跳转到表格下方的完整定义。

执行规则如下：

1. **核心术语在当前位置解释**：某个术语第一次真正参与知识推导时，在该段正文中先给新手可复述的解释，再给准确专业定义，并立即结合当前代码、执行过程或构建产物说明。
2. **补充术语允许链接查阅**：若某个次要术语的索引定义已经足够，正文首次有效使用时必须链接到对应的 Markdown 标题，例如 `[重定位](#术语-d04-13-重定位)`，使学习者可以原地跳转。
3. **索引统一使用两列表格**：第一列使用 `[术语](#对应标题)` 形成可点击入口，第二列只放一句话直观解释，使索引紧凑且便于快速扫描。
4. **使用真正的 Markdown 标题作为目标**：每个术语的完整定义仍放在表格下方，并使用六级标题，例如 `###### 术语-d04-13-重定位`。标题只使用中文、英文小写字母、数字和连字符，正文及索引中的片段必须与标题文字完全一致。
5. **禁止伪兼容锚点**：不得使用 `<a id="..."></a>` 作为 Obsidian 跳转目标，也不得把表格中的单独一行作为跳转目标。Markdown 表格只承担导航，真正的目标必须是表格下方的 Markdown 标题。
6. **链接不能掩盖讲解缺失**：仅当索引定义足以支撑当前段落时才使用链接；当前推导依赖该概念的规则、边界或易混点时，必须在正文直接解释。
7. **避免无效术语堆积**：若一个词在正文中没有实际使用或解释位置，就不应仅为了扩大术语索引而列为“关键术语”。
8. **重复出现不必重复定义**：首次建立正确概念后，后文可以直接使用；需要回顾时再次链接即可。
9. **发布前验证真实目标**：检查每个 `#...` 片段都存在同名 Markdown 标题，并确认不存在旧式 HTML 锚点或孤立链接；字符串相同不等于 Obsidian 已能索引。
...
- 完整知识地图
- 今天掌握 / 先看全貌 / 后续深入
- 正文中的术语解释 + 两列表格形式的可跳转术语索引 + 表格下方的完整定义
- 核心规则与准确表述
- 最小示例
- 执行、内存或构建过程跟踪
- 常见错误与失败实验
