...
每份讲义可以在正文前设置“关键术语索引”，但它只承担查阅和复习功能，不能代替正文中的教学过程。学习者不需要先背完整个索引，才可以继续阅读正文。
...
3. **使用真正的 Markdown 标题作为目标**：每个术语条目使用六级标题，例如 `###### 术语-d04-13-重定位`。标题只使用中文、英文小写字母、数字和连字符，正文片段必须与标题文字完全一致。
4. **禁止伪兼容锚点**：不得使用 `<a id="..."></a>` 作为 Obsidian 跳转目标，也不得尝试链接到 Markdown 表格中的单独一行。两者都不能作为 GitHub 与 Obsidian 的共同可靠格式。
5. **链接不能掩盖讲解缺失**：仅当索引定义足以支撑当前段落时才使用链接；当前推导依赖该概念的规则、边界或易混点时，必须在正文直接解释。
6. **避免无效术语堆积**：若一个词在正文中没有实际使用或解释位置，就不应仅为了扩大术语索引而列为“关键术语”。
7. **重复出现不必重复定义**：首次建立正确概念后，后文可以直接使用；需要回顾时再次链接即可。
8. **发布前验证真实目标**：检查每个 `#...` 片段都存在同名 Markdown 标题，并确认不存在旧式 HTML 锚点或孤立链接；字符串相同不等于 Obsidian 已能索引。
...
- 完整知识地图
- 今天掌握 / 先看全貌 / 后续深入
- 正文中的术语解释 + 可跳转术语索引（新手直觉、专业定义和易混边界）
- 核心规则与准确表述
- 最小示例
- 执行、内存或构建过程跟踪
- 常见错误与失败实验
