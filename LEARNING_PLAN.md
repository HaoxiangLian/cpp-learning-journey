...
每份讲义可以在正文前设置“关键术语索引”，但它只承担查阅和复习功能，不能代替正文中的教学过程。学习者不需要先背完整张表，才可以继续阅读正文。

执行规则如下：

1. **核心术语在当前位置解释**：某个术语第一次真正参与知识推导时，在该段正文中先给新手可复述的解释，再给准确专业定义，并立即结合当前代码、执行过程或构建产物说明。
2. **补充术语允许链接查阅**：若某个次要术语的表格定义已经足够，正文首次有效使用时必须写成指向对应表格锚点的链接，例如 `[重定位](#term-d04-13)`，使学习者可以原地跳转。
3. **表格条目具有稳定锚点**：锚点统一使用 `term-dNN-XX` 形式，正文链接与表格定义必须一一对应，并兼容 GitHub 与 Obsidian 的同页跳转。
4. **链接不能掩盖讲解缺失**：仅当表格定义足以支撑当前段落时才使用链接；当前推导依赖该概念的规则、边界或易混点时，必须在正文直接解释。
5. **避免无效术语堆积**：若一个词在正文中没有实际使用或解释位置，就不应仅为了扩大术语表而列为“关键术语”。
6. **重复出现不必重复定义**：首次建立正确概念后，后文可以直接使用；需要回顾时再次链接即可。

术语索引至少包含：

| 栏目 | 作用 |
|---|---|
| 术语 | 给出中文名称，必要时附英文名称或实际 C++ 拼写，并设置稳定锚点 |
| 新手先这样理解 | 用一句话建立正确直觉 |
