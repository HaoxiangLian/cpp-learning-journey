...
默认实参通常只在一个可见声明处写一次，避免声明与定义之间重复或不一致。
...
**代码与机制**：每次递归调用都是一次新的函数调用，拥有自己的形参对象和返回过程。主流实现通常为尚未返回的调用保存调用记录，常称为调用帧并放在运行时调用栈中；这是常见实现模型，不是 C++ 标准规定的唯一物理布局。标准也不保证尾递归一定被优化。
