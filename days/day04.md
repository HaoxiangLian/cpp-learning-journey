...
1. 从调用点分清实参、形参和返回值各自承担的作用；
2. 用“是否建立了独立形参对象”解释值传递；
3. 区分函数重载与默认实参，并判断常见调用会匹配哪个函数；
4. 检查递归是否既有终止条件，又在每次调用中向终止条件推进。
...
把 `add_one(original)` 的变化按时间写开：

| 时刻 | 调用方的 `original` | 函数内的 `count` | 调用方的 `changed` |
|---|---:|---:|---:|
| 调用前 | `3` | 尚未创建 | 尚未得到结果 |
| 进入函数 | `3` | 用实参值初始化为 `3` | 尚未得到结果 |
| 执行 `++count` | `3` | `4` | 尚未得到结果 |
| 函数返回后 | `3` | 已结束 | 用返回值初始化为 `4` |

初学者先记住：源代码中要按“两个独立对象”推理，所以改 `count` 不会倒流去改 `original`。

进阶边界：“语言语义”在这里指 C++ 要求程序表现得符合上表；“机器实现”才讨论处理器是否真的把一个整数从某个内存位置复制到另一个位置。编译器可以省掉不影响结果的机器步骤，但不能把 `original` 的最终值擅自改成 `4`。类对象的值传递还会涉及拷贝、移动和省略，第 10—11 天再展开；不要把今天对 `int` 的结论扩大成所有类型都“零成本”或“很昂贵”。
...
普通话区别：重载是“同名但确实有多个函数，由实参类型选择一个”；默认实参是“仍然只有一个函数，只替调用者补上省略的输入”。
...
普通话结论：函数为了解决 `n`，先让同一个函数去解决更小的 `n - 1`；当 `n` 已经不大于 `0` 时直接回答，不再继续调用。

**概念落点**：[递归（recursion）](../docs/glossary/day04.md#递归recursion)是函数在执行过程中直接或间接再次调用自身的调用结构。

先只看每一层“正在等什么”：

| 尚未完成的调用 | 本层的 `n` | 必须等到什么结果才能返回 |
|---|---:|---|
| `sum_to(3)` | `3` | 等 `sum_to(2)`，再计算 `3 + ...` |
| `sum_to(2)` | `2` | 等 `sum_to(1)`，再计算 `2 + ...` |
| `sum_to(1)` | `1` | 等 `sum_to(0)`，再计算 `1 + ...` |
| `sum_to(0)` | `0` | 直接返回 `0` |

`sum_to(0)` 返回后，等待中的调用按相反方向继续：先得到 `1`，再得到 `3`，最后得到 `6`。初学者先记住：每次调用都有自己的 `n`，较外层调用在较内层调用返回前还没有完成。

**代码与机制**：程序运行时必须为每个尚未完成的调用记住必要信息，例如本层的 `n`，以及内层返回后要继续计算哪一部分。这组信息常称为“调用记录”或“调用帧”。主流桌面环境通常把这些记录按“后调用的先返回”组织在调用栈中。

这是第二遍才需要的实现说明：C++ 规则要求每次调用表现得像拥有自己的形参和返回过程，但没有规定调用记录必须采用某种固定内存布局，也不保证尾递归一定被优化。第一遍做题只要能写出上表，就足以判断递归是否终止和结果怎样组合。
...
**完整答题逻辑**：先定位函数两侧，再按“调用前、进入函数、修改形参、返回后”说明状态：`original` 始终为 `3`，独立形参 `count` 从 `3` 变为 `4`，返回值再初始化 `changed`。最后才补充实现边界：C++ 要求程序符合这种独立对象语义，但不要求机器代码一定出现一条肉眼可见的内存复制指令。
...
- 能用“每层的输入、等待的结果、返回顺序”跟踪递归，再说明调用栈只是保存未完成调用的常见实现方式。
