...
1. 安全遍历固定大小数组，并说明数组与指针为什么不是同一种对象；
2. 区分依赖终止空字符的 C 风格字符串与自行管理长度的 `std::string`；
3. 配对 `new/delete` 与 `new[]/delete[]`，识别越界、泄漏、释放后使用和重复释放；
4. 说明 `malloc/free` 管理的内容为何不能与 `new/delete` 混用。
...
参考方向：局部指针可以指向静态或动态对象；还需证明指针来自合法初始化、指向类型合适且仍在生命周期内的对象，并且这次访问没有越界；释放后访问属于未定义行为，不保证固定症状。

> 你的记录（校正）：判断重点不是“系统有没有回收这块内存”。即使原地址中的字节看起来还在，只要对象的生命周期已经结束，也不能继续把它当作原对象访问。
...
`readings` 是一个数组对象，里面有三个 `int` 元素；它不是一个指针对象。只有在许多表达式中使用 `readings` 时，语言才会临时得到“指向第一个元素”的指针值。
...
> [!NOTE]
>  ### 什么是“显式界限”？
> 这里特指 `int values[N];` 这种内建数组声明：如果在方括号里明确写出 `N`，它就是显式界限。
> ### 什么是“常量表达式”？
> 对这种内建数组声明，`N` 必须能按 C++17 的常量表达式规则在编译阶段确定。运行时数量应交给 `std::vector`，或像本日后半部分那样使用动态分配；不要把某些编译器支持的变长数组扩展当作标准 C++17。
...
**小检查**：为什么 `int values[10]` 与 `int* values` 在普通对象声明中是不同类型，却在函数形参列表中可能表示相同的调整后参数类型？
	~~数组在作为实参传入函数后~~，被声明为数组的形参，会自动转换为指向数组首元素的指针；
...
先看差别：`label` 的第四个元素是值为零的字符 `\0`，旧式字符串函数靠它判断“到这里结束”；`name` 则能直接用 `name.size()` 得到长度 `3`。所以“字符数组”只有满足终止约定时，才能当作 C 风格字符串使用。
...
先按成对动作阅读：第一组创建一个 `int`，所以用 `delete`；第二组创建三个元素的数组，所以用 `delete[]`。方括号是否出现必须匹配，且每个创建结果只释放一次。
...
分类：**未定义行为**，不保证某次测试一定失败。修复语法是匹配 `delete[]`，工程修复则是减少裸 `new`：数组优先交给==标准容器==管理，单对象所有权优先交给==智能指针==管理。容器与智能指针分别在第 16、18 天展开。

**小检查**：`delete[] buffer; buffer = nullptr;` 中，哪一步结束对象生命周期并释放存储？哪一步只改变指针变量？
	delete[] buffer结束对象生命周期并释放内存；后一步改变指针变量；
...
```cpp
int* value{new int{7}};
delete value;                    // new 家族配 delete 家族

void* raw{std::malloc(128)};
std::free(raw);                  // malloc 家族配 free
```

普通话规则先记牢：从哪套接口取得，就交还给同一套接口。即使某个实现内部复用了相同的底层分配器，也不能改变调用者应遵守的公开配对。
...
上面的 `malloc` 示例需要包含 `<cstdlib>`。实际使用返回值前还要检查它是否为 `nullptr`；这里只展示分配族配对，对象生命周期与类型化访问需要额外规则。
