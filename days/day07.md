...
参考方向：局部指针可以指向静态或动态对象；==还需证明类型、边界和生命周期(这个非空地址是通过合法手段申请来的(野指针）、没有越界（越界访问）、且该内存至今仍未被系统回收（悬垂指针）。)；==释放后访问属于未定义行为，不保证固定症状。
...
> [!NOTE]
>  ### 什么是“显式界限”？
> 当你定义一个数组时，如果你明确在方括号里写了一个数字，这就叫“显式界限”。
> ### 什么是“常量表达式”？
> 这个数字必须在“编译阶段”就能够被完全计算并确定下来。绝对不能等到程序“运行阶段”再去决定。
