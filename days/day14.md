...
**小检查**：`larger(4, 9)` 中==模板参数、模板实参和两个函数调用实参分别是什么==？为什么 `larger(4, 9.0)` 不能按同一规则推导出唯一 `T`？
	模板参数是T，模板实参是int，4，9；不能，模板参数推导出分别为int和double，无法推出唯一T
...
可以找到合适的模板实参int，而且存在有效的小于比较操作；不是，是由类型支持小于比较操作才可以；
...
> |能不能继承、多态、模板|都可以|都可以|
> 
> struct 偏数据，class 偏封装；语法能力基本相同，主要区别是默认访问权限。
> - `struct`：适合**单纯保存数据**，强调“数据集合”
> - `class`：适合**封装对象行为**，有私有数据、接口、复杂逻辑
