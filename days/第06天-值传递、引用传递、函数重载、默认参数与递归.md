...
| 术语 | 新手先这样理解 | 专业上更准确地说 |
|---|---|---|
| <a id="term-d06-01"></a>**值形参（by-value parameter）** | 函数拿到一份独立的参数值，修改形参通常不改调用者对象 | 值形参本身是参数对象，由实参按初始化规则建立。对类类型可能发生复制、移动或省略；“一定完整复制一份内存”不是普遍准确的说法。 |
| <a id="term-d06-02"></a>**引用（reference）** | 给已有对象提供另一个访问名称 | 引用是初始化后指代某个对象或函数的实体，通常不能重新改绑。使用引用表达式时通常是在访问所指代的实体；引用不是一个可以像普通对象那样独立赋值改绑的容器。 |
| <a id="term-d06-03"></a>**绑定（binding）** | 确定这个引用现在指向哪个实体 | 引用绑定是引用初始化的一部分，语言规则会检查对象类别、类型转换和 <code>const</code> 限定。它与以后通过引用给对象赋值是两件事。 |
| <a id="term-d06-04"></a>**左值（lvalue）** | 通常表示一个有身份、可以再次定位到的对象 | 左值是一种表达式值类别，通常标识一个对象或函数。它并不等同于“等号左边的值”；有些左值不能被修改，某些赋值左操作数还需满足额外条件。第 14、22 天继续深化。 |
| <a id="term-d06-05"></a>**非常量左值引用形参 <code>T&amp;</code>** | 函数直接操作调用者提供的可修改对象 | 这种形参必须按引用绑定规则绑定到合适的非 <code>const</code> 左值；通过形参修改所指代对象会被调用者观察到。字面量通常不能直接绑定给它。 |
| <a id="term-d06-06"></a>**只读引用形参 <code>const T&amp;</code>** | 函数借用对象来读取，但不能通过该形参修改它 | 该引用禁止通过自身修改所指代对象，并能绑定更多种实参。某些类型转换可能产生临时对象，因此“使用 <code>const T&amp;</code> 永远不创建临时对象”并不准确。 |
| <a id="term-d06-07"></a>**函数重载（overloading）** | 多个同名函数处理不同类型的输入 | 在允许的作用域和规则下，多个同名函数可形成重载；调用时编译器依据名称查找、实参和转换规则选择目标。仅形参名不同或仅返回类型不同不能构成普通函数重载。 |
| <a id="term-d06-08"></a>**重载集合（overload set）** | 当前名字对应的一组同名函数 | 名称查找在当前上下文中找到的相关函数形成供后续重载决议考虑的集合。它不是一个运行时容器。 |
| <a id="term-d06-09"></a>**重载决议（overload resolution）** | 编译器从同名函数中选出这一调用该用哪个 | 重载决议发生在编译期，依次考虑候选函数、可行性和隐式转换序列的优劣；只有存在唯一最佳可行函数时调用才成立。 |
| <a id="term-d06-10"></a>**候选函数（candidate function）** | 有资格进入本轮选择名单的函数 | 候选函数由名称查找及重载规则确定。进入候选集合并不表示它能接受当前实参。 |
| <a id="term-d06-11"></a>**可行函数（viable function）** | 参数数量和类型规则都允许这次调用的候选函数 | 可行性要求实参数量满足规则，并且每个实参都存在可接受的隐式转换或引用绑定到对应形参。 |
| <a id="term-d06-12"></a>**最佳可行函数（best viable function）** | 所需转换整体上唯一更合适的那个函数 | 编译器按标准规定的偏序与转换等级比较可行函数；“看起来最接近”只是直觉，必须存在规则意义上的唯一最佳者。 |
| <a id="term-d06-13"></a>**歧义（ambiguity）** | 编译器无法唯一决定选哪个函数 | 当存在多个可行函数却没有唯一最佳选择时，调用在编译期失败。编译器不会随机挑选，也不会等到运行时再决定。 |
| <a id="term-d06-14"></a>**隐式转换（implicit conversion）** | 编译器按规则自动把实参调整为形参需要的类型 | 隐式转换是语言在特定上下文允许自动进行的转换序列。它的等级会影响重载选择，但并非“能转就一定安全或一定会选中”。 |
| <a id="term-d06-15"></a>**默认实参（default argument）** | 调用时省略某项输入，由声明提供默认表达式 | 默认实参附着在可见声明上，在调用点用于补足省略的实参，并在每次调用时求值；它不是函数体内部自动初始化形参的隐藏语句，也不属于函数类型。 |
| <a id="term-d06-16"></a>**递归（recursion）** | 函数直接或间接调用自己 | 每次递归调用都是一次新的函数调用，具有独立的形参和局部对象状态；递归必须有可达的终止路径，且问题状态需要朝该路径推进。 |
| <a id="term-d06-17"></a>**基例（base case）** | 不再继续调用自己、直接结束或给出结果的情况 | 基例是递归控制流中的终止分支。只写出基例还不够，递归步骤必须能在有限次调用内到达它。 |
| <a id="term-d06-18"></a>**栈溢出（stack overflow）** | 递归太深，常见调用栈空间耗尽 | 栈溢出是常见实现上的运行时故障，不保证以某个标准 C++ 异常形式出现。无限递归通常先表现为持续调用，随后可能异常终止。 |
...
- [重载决议](#term-d06-09)决定同名函数中究竟调用哪一个
- [默认实参](#term-d06-15)在调用处补足省略的实参，然后再进行函数调用
...
引用为已有对象提供另一个名字。引用必须在初始化时[绑定](#term-d06-03)，之后不能改为绑定另一个对象。

### 3. [非常量左值引用形参](#term-d06-05)
...
字面量 `40` 不是可以绑定到 `int&` 的可修改[左值](#term-d06-04)，因此这是编译错误。
...
如果递归调用层数持续增长，常见实现中的调用栈空间可能耗尽，这种运行时故障称为[**栈溢出**](#term-d06-18)。C++ 标准不保证它一定以可捕获异常的形式出现，程序也可能直接异常终止。
