...
1. 说明创建类对象时为什么会选择构造函数，并用成员初始化列表建立初始状态；
2. 根据成员在类中的声明顺序预测初始化顺序；
3. 说明析构函数在对象结束过程中何时运行，并预测成员的逆序析构；
4. 使用 `explicit` 阻止不符合接口意图的隐式转换。
...
参考方向：有效对象在公开操作完成前后应满足不变量；类成员可访问该类私有成员（**一个类里面定义的成员函数，可以访问这个类自己的 private 成员。**）；局部对象离开对应范围时会按语言规则销毁，其析构过程与动态对象的 `delete` 路径要分开分析。
...
> [!NOTE]
> 在 C++ 的类中，**默认成员初始化器（default member initializer）**，就是：
> 
> > **在类的成员变量声明处，直接给这个成员写一个默认初始值。**
...
> [!NOTE] explicit的定义
> `explicit` 是一个访问/行为修饰符。当它被放在类构造函数的声明之前时，它明确告诉编译器：**这个构造函数只能被显式调用，绝对不允许参与隐式类型转换或拷贝初始化。**
> [[notes for day09_5]]
