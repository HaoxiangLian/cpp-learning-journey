...
> 函数相关术语常成对出现。先看“声明与定义”“形参与实参”“返回类型与返回值”的区别，再进入正文。

| 术语 | 新手先这样理解 | 专业上更准确地说 |
|---|---|---|
| **函数（function）** | 给一段可重复执行的行为起名字 | 函数是可被调用的程序实体，具有函数类型；定义时还具有函数体。函数不是对象，也不是“保存代码的变量”。 |
| **函数类型（function type）** | 描述函数接收什么、返回什么的类型信息 | 函数类型由返回类型和形参类型列表等规则形成，例如“接收两个 <code>int</code> 并返回 <code>int</code>”。函数名、形参名和默认实参不属于函数类型。 |
| **函数接口（interface）** | 调用者为了正确使用函数需要知道的约定 | “接口”是工程概念，通常包括函数声明表达的名称、参数类型、返回类型，以及文档中的前置条件、效果和错误约定；它不等同于单独一个 C++ 关键字。 |
| **函数声明（declaration）** | 先告诉编译器“有这样一个函数，可以这样调用” | 函数声明把函数名称和类型等信息引入作用域，使调用点能够进行名称查找和类型检查。只有声明而没有可用定义时，程序仍可能在链接阶段失败。 |
| **函数定义（definition）** | 给出函数真正执行的代码 | 函数定义是带函数体的声明，它为函数提供实现。对普通非内联函数，一个程序中通常只能有一个符合 ODR 的定义；完整规则在第 17 天学习。 |
| **函数调用表达式（function call expression）** | 写函数名和括号，让函数执行一次 | 调用表达式先确定被调用函数，处理各个实参与形参的初始化，再把控制转移给被调用函数；有返回值时，整个调用表达式会产生相应结果。 |
| **调用者与被调用者（caller and callee）** | 发起调用的是调用者，被叫到的是被调用者 | 这是描述调用关系的工程术语。例如 <code>main</code> 执行 <code>add(2, 3)</code> 时，<code>main</code> 是 caller，<code>add</code> 是 callee。 |
| **形参（parameter）** | 函数定义中用于接收输入的名字 | 形参在函数声明或定义的形参列表中声明，属于被调用函数。调用发生时，它会按参数传递规则由对应实参初始化或绑定。 |
| **实参（argument）** | 调用函数时实际放进括号的表达式 | 实参是调用表达式提供的表达式，例如 <code>add(a, 3)</code> 中的 <code>a</code> 和 <code>3</code>。实参与形参不是同一个语法位置，也不一定是同一个对象。 |
| **返回类型（return type）** | 函数承诺返回哪一类结果 | 返回类型是函数声明的一部分，并参与函数类型的形成；普通函数不能只靠返回类型不同来构成重载。 |
| **返回值（return value）** | 某一次调用实际交回给调用者的结果 | 对非 <code>void</code> 函数，<code>return</code> 表达式的结果要按返回类型规则进行检查和必要转换，然后用于形成该次调用的结果。返回类型与返回值不能混称。 |
| **<code>void</code> 返回** | 这个函数完成动作，但不交回一个普通值 | 当函数返回类型为 <code>void</code> 时，调用表达式不产生可供普通值计算使用的结果。<code>void</code> 是语言中的类型，不应简单解释成数字 0 或空指针。 |
| **控制转移（control transfer）** | 程序暂时跳到函数体，结束后再回来 | 调用把控制交给被调用函数；正常返回时，控制继续到调用表达式之后。异常、程序终止等路径可能不会按普通方式返回。 |
| **调用栈（call stack）** | 运行时按调用顺序保存“回来位置和局部状态”的常见结构 | 调用栈是多数实现采用的运行时机制，不是 C++ 标准强制规定的物理结构。优化器可以内联函数，使某次调用不产生可观察的独立栈帧。 |
| **栈帧（stack frame）** | 某一次函数调用在调用栈中的那一层 | 栈帧是实现层术语，可能保存返回地址、局部对象、溢出到内存的形参和寄存器状态；其具体布局由平台 ABI、编译器和优化决定。 |
| **函数签名（function signature）** | 用来区分函数身份的一组类型信息 | 在 C++ 标准语境中，“签名”有特定定义，不能随意当作“整条函数声明”的同义词；普通函数签名通常不包含返回类型和形参名。本课程会在涉及重载时谨慎使用该词。 |
