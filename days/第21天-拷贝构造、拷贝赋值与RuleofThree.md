...
两个 `Buffer` 对象若指向同一块动态数组，任意一个析构都可能让另一个悬空。复制的核心问题不是语法，而是复制后两者是否拥有独立、有效的状态。

### 2. 先看这几行代码

```cpp
Buffer first{4};
Buffer second{first};       // 复制构造：建立新对象
Buffer third{2};
third = first;              // 复制赋值：修改已存在对象
```
...
| 顺序 | 你必须能指出的具体变化 |
|---|---|
| 1 | 复制构造为 `second` 建立独立资源并复制元素 |
| 2 | 复制赋值先准备新资源，再安全替换 `third` 的旧资源 |
| 3 | 编译器生成的逐成员复制对裸拥有指针只复制地址，形成浅复制 |
| 4 | 自赋值时必须保持对象有效 |
| 5 | 若成员本身是 `vector`、`string`、智能指针等 RAII 类型，通常优先 Rule of Zero |

如果不能复述上表，就修改一个输入、手写预测并运行 [本课可运行示例](../examples/day-21/main.cpp)；不要靠继续读更多术语解决。

### 4. 只给已经看到的现象命名

| 核心概念 | 先用人话理解 | 回到代码或命令找证据 |
|---|---|---|
| **复制构造** | 用源对象建立一个新对象。 | `Buffer second{first};` |
| **复制赋值** | 目标已存在，替换其值但不重新开始生命周期。 | `third = first;` |
| **浅复制/深复制** | 裸拥有指针的逐成员复制只复制地址；独立值语义需要独立资源。 | 两个 `data_` 是否指向同一数组 |
| **Rule of Zero/Three** | 优先用资源成员获得正确复制；必须手管资源时一起审查析构、复制构造、复制赋值。 | `vector` 成员 vs 裸指针 |

这些解释是第一遍可用模型。第二遍会补边界和专业规则；补细节不等于推翻这里的主线。

### 5. 先形成一个求职可用回答

> 复制构造用于从源对象建立新对象，复制赋值修改已经存在的目标对象。拥有裸资源的类若使用默认逐成员复制，通常只复制地址，导致共享同一释放责任。深复制要创建独立资源；赋值还要处理目标旧资源、异常安全和自赋值。工程上优先让 `vector`、`string` 等成员管理资源，使编译器生成的复制行为正确。
...
不以手写裸数组类作为最终工程方案；它用于看清浅复制，实际优先 Rule of Zero。

暂缓不等于删除：第二遍会明确展开，课程计划也标出了对应单元。

### 7. 必须动手完成

- 可运行示例：[main.cpp](../examples/day-21/main.cpp)
- 配套练习：[day-21.md](../exercises/day-21.md)
- 编程任务：实现一个以 `std::vector<int>` 为成员的 `Buffer`，验证默认复制产生两个独立序列，并说明为什么这是 Rule of Zero。
预期结果：
...
- 故障实验：默认复制只复制哪个成员值？使用 ASan 观察重复释放；给出深复制、禁止复制或改用 `vector` 三种设计方向。
