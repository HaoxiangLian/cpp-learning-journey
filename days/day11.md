...
1. 先比较深拷贝与转移后的资源数量和源对象状态，再解释移动构造的作用，并说明 `std::move` 本身不搬运资源；
2. 区分移动构造与移动赋值，正确处理目标旧资源和源对象的移后状态；
3. 用 Rule of Five 审计特殊成员路径，并优先用 Rule of Zero 简化业务类。
...
### 机制一：先比较复制与转移，再认识移动构造和 `std::move`

**实际问题**：下面的 `source` 独占数组 A。我们要创建 `target`，并且之后不再需要 `source` 保存原数据。Day 10 的深拷贝会申请数组 B 并复制元素；今天先观察另一种目标状态：不创建数组 B，让新对象接管数组 A。
...
暂时把 `std::move(source)` 整体看成“调用方愿意交出 `source` 的资源”；它的准确作用会在状态表之后定义。先比较 Day 10 的深拷贝与今天想得到的转移结果：

| 比较项 | 深拷贝创建新对象 | 转移创建新对象 |
|---|---|---|
| 源对象原来拥有 | 数组 A | 数组 A |
| 目标对象最后拥有 | 新数组 B，元素值来自 A | 原数组 A |
| 底层数组数量 | 两个 | 仍是一个 |
| 操作后源对象 | 仍拥有 A，内容不变 | 仍在生命周期内，但不再拥有 A；具体状态由类型约定 |

再只看本例这次操作前后的所有权，不急着背函数名：
...
**概念落点**：现在按“为什么需要另一种入口 → 入口叫什么 → 调用点怎样选择它”的顺序引入三个相邻概念，而不是一次背三个名字。

1. 深拷贝构造的参数通常是 `const T&`，它不能表示“允许修改来源并交出资源”这层调用意图。为了让重载选择区分这种来源，C++ 提供[右值引用（rvalue reference）](../docs/glossary/day11.md#右值引用rvalue-reference)：右值引用是以 `T&&` 表示、能够绑定到相应右值并让重载选择区分可被转移来源的引用类型。
2. 构造函数采用这个参数形式后，就得到本机制真正要学习的[移动构造函数（move constructor）](../docs/glossary/day11.md#移动构造函数move-constructor)：移动构造函数是以同类右值引用初始化新对象的构造函数，通常通过接管源对象可转移资源并把源对象留在满足其契约的有效状态来建立目标。
3. 有名字的变量 `source` 不会仅因“以后不用了”就自动选择该入口。调用方可写 [`std::move`](../docs/glossary/day11.md#stdmove)：`std::move` 是把实参显式转换为相应右值引用表达式的标准库函数；它本身不搬运资源，后续被选择的操作决定是否以及怎样移动。

这里先把 `T&&` 当成“这个重载愿意接收可转移来源”的参数形式，把 `std::move(source)` 当成调用方对重载选择的显式请求。表达式值类别的完整体系留到 Day 20；今天无需用尚未掌握的术语链解释它。
...
**小检查**：深拷贝与本例移动完成后各有几块底层数组？在 `MoveOnlyBuffer target{std::move(source)};` 中，哪一部分只影响重载选择，哪一部分真正修改 `source.data_`？
	深拷贝完成后又两块底层数组，移动后有一块底层数组； 'std::move(source)' 影响重载选择；移动构造函数真正修改source.data_; 
### 机制二：移动赋值要先结束目标的旧所有权
