...
`const`、`constexpr`、花括号初始化和类型转换经常被一起问，是因为它们共同决定“一个值何时确定、能否修改、能否安全放入目标类型”。
...
double sensor_value{179.8};
int displayed{static_cast<int>(sensor_value)};
```
...
| 顺序 | 你必须能指出的具体变化 |
|---|---|
| 1 | `joint_count` 必须能在常量求值中建立，适合数组界限等编译期场景 |
| 2 | `safety_limit` 不能通过这个对象名称修改，但不因此自动成为所有语境的编译期常量 |
| 3 | 花括号初始化会拒绝某些明显窄化 |
| 4 | `static_cast<int>` 明确要求转换，但小数部分仍会丢失 |
| 5 | 指针上的 `const` 要区分指针本身是否可改和所指对象是否可改 |

如果不能复述上表，就修改一个输入、手写预测并运行 [本课可运行示例](../examples/day-13/const_conversion_demo.cpp)；不要靠继续读更多术语解决。

### 4. 只给已经看到的现象命名

| 核心概念 | 先用人话理解 | 回到代码或命令找证据 |
|---|---|---|
| **const** | 通过该名字或接口不允许修改对象；不等于对象永远不可能变化。 | `const double limit` |
| **constexpr** | 要求值或函数可参与常量表达式求值；是否真在编译期求值取决于使用位置。 | 数组大小或 `static_assert` |
| **初始化** | 不同初始化语法有不同规则，花括号会拒绝一部分窄化。 | `int value{3.5};` 失败 |
| **类型转换** | 把值按目标类型规则表示；可能丢精度、范围或限定。 | `static_cast<int>(angle)` |

这些解释是第一遍可用模型。第二遍会补边界和专业规则；补细节不等于推翻这里的主线。

### 5. 先形成一个求职可用回答

> `const` 主要表达不可通过该访问路径修改，`constexpr` 要求对象或函数可参与常量求值；两者相关但不等价。初始化发生在对象建立时，赋值发生在对象已经存在后。转换需要同时检查来源类型、目标类型和值域，显式 `static_cast` 只是表达意图，不保证无损。面试时我会用实际数值说明窄化，而不是只背转换名称。
...
不背全部隐式转换表；先识别是否丢信息、是否改变 const 和是否明确表达意图。

暂缓不等于删除：第二遍会明确展开，课程计划也标出了对应单元。

### 7. 必须动手完成

- 可运行示例：[const_conversion_demo.cpp](../examples/day-13/const_conversion_demo.cpp)
- 配套练习：[day-13.md](../exercises/day-13.md)
- 编程任务：实现安全的传感值显示：输入 `double`，检查范围后显式转换为 `int`；用 `constexpr` 保存关节数，用 `const` 保存运行期安全上限。
预期结果：
...
- 故障实验：记录列表初始化拒绝窄化的编译诊断。修复不能只改成圆括号而不解释信息损失。
